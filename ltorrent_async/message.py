...
class MessageDispatcher:
    ...
    async def dispatch(self):
        try:
            ...
            await self.stdout.WARNING("Received keep alive message.")
            return None
        except Exception as e:
            await self.stdout.ERROR("Error when unpacking message : %s: %s" % (e, self.payload))
            return None
...
class UdpTrackerAnnounce(Message):
    ...
    def to_bytes(self):
        ...
        msg = (
            ...
            self.info_hash + peer_id + downloaded +
            left + uploaded + event +
...
class Handshake(Message):
    ...
    def to_bytes(self):
        reserved = b'\x00' * 8
        handshake = pack(
            ">B{}s8s20s20s".format(HANDSHAKE_PSTR_LEN),
            HANDSHAKE_PSTR_LEN,
            HANDSHAKE_PSTR_V1,
            reserved,
            self.info_hash,
    ...
    def from_bytes(cls, payload):
        ...
        pstr, _, info_hash, peer_id = unpack(">{}s8s20s20s".format(pstrlen), payload[1:cls.total_length])
...
class Have(Message):
    ...
    @classmethod
    def from_bytes(cls, payload):
        _, message_id, piece_index = unpack(">IBI", payload[:cls.total_length])
        if message_id != cls.message_id:
...
class Request(Message):
    ...
    @classmethod
    def from_bytes(cls, payload):
        _, message_id, piece_index, block_offset, block_length = unpack(
            ">IBIII",
...
class Piece(Message):
    ...
    def to_bytes(self):
        return pack(
            ">IBII{}s".format(self.block_length),
            self.payload_length,
            self.message_id,
            self.piece_index,
            self.block_offset,
    ...
    def from_bytes(cls, payload):
        ...
        _, message_id, piece_index, block_offset, block = unpack(
            ">IBII{}s".format(block_length),
